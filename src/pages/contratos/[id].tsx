...
export default function CtoDetalhes() {
    const [contrato, setContrato] = useState<Array<ContratoProps>>([]);

    const router = useRouter();
    const [idCto, setIdCto] = useState(router.query.id);
    ...
    useEffect(() => {    
        ...
        let buscalUrl = `https://webio.aparecida.go.gov.br/api/lict/contratos/${idCto}`;
    ...
    const itemsSubmenu = [
        {
            menId: '1',
            menTitle: 'Detalhes Contrato',
            breadcrumbs:[
              {descricao: "Portal da Transparencia", link: "/portaltransparencia"},
              {descricao: "Contratos", link: "/contratos"},
            ]
        },
    ] 
    ...
    return (
        <div className=" ">
            ...
            <div className=''>
                {contrato.map((row) => (
                <div key={row.id} className='md:ml-20 md:mr-20'>                
                    <div className='flex flex-row justify-between items-center mt-3 border-b-2 border-gray-200 py-2 mb-11'> 
                        <div className='w-full h-auto ml-2 text-md font-bold md:text-2xl'>
                            {row.titulo} - {row.id}
                        </div>
                    ...
                    <div key={anexos.id} className="grid grid-cols-4 gap-1 md:grid-cols-7 md:gap-2 h-auto p-2">                        
                        <div className='col-span-1'>{anexos.data}</div>
                        <div className='col-span-2 md:col-span-5 text-[14px]'>{anexos.nome}</div>
                    ...
                    <div className="grid grid-cols-1 gap-1 md:grid-cols-4 md:gap-4 h-auto ">
                        ...
                        <div key={lic.id} className="bg-white text-black border border-[#008C3D] hover:bg-[#008C3D] hover:text-white flex flex-row items-center justify-center ml-2 mr-2 md:ml-0 md:mr-0 w-auto p-2 rounded-lg" >
                    ...
                    <div className='w-full border-b-2 border-gray-200 py-2 mt-40 mb-6 p-2'>
                        <span className='text-[#008C3D] font-bold mt-20'>CONTRATOS ASSOCIADOS</span>
                    </div>
                    <div className="grid grid-cols-1 gap-1 md:grid-cols-4 md:gap-4 h-auto ">
                        {row.contratos_associados.map((cto) => (
                        <div key={cto.id} className="bg-white text-black border border-[#008C3D] hover:bg-[#008C3D] hover:text-white flex flex-row items-center justify-center ml-2 mr-2 md:ml-0 md:mr-0 w-auto p-2 rounded-lg" >
                            <a target="_blank" href="#">{cto.nome}</a>
                        </div>
                        ))}
                    </div>                    
                </div>

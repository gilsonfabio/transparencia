...
export default function Contratos({categorias, fornecedores, entidades}: apiProps) {
    ...
    //const [fornecedores, setFornecedores] = useState<Array<FornecedorProps>>([]);
    ...
    useEffect(() => {            
        ...
        axios({
            method: 'post',    
            url: `https://webio.aparecida.go.gov.br/api/lict/v1/filtrar/contratos`,
        ...
        //api.get("https://webio.aparecida.go.gov.br/api/lict/contratos/categorias")
        //    .then(res => {
        //    setTipos(res.data)             
        //}).catch((err) => {
                        ...
                        <div className='w-72 mt-0 mb-3'>                            
                            <div>
                                <li className={`accordion_item ${clickedSit ? "active mb-2" : ""} list-none`}>
                                    ...
                                    <div className={`answer_wrapper ${clickedSit ? "active p-2 mb-5 bg-gray-200 dark:bg-gray-800 border-l-2 border-gray-400 h-96 overflow-y-scroll" : "hidden"}`}> 
                    ...
                    <div className='flex flex-row justify-between items-center w-full text-black p-2 bg-[#F3F3F3] dark:bg-gray-800'> 
                        ...
                        <div className='flex flex-col w-full h-full text-black'>
                            ...
                            {itens.map((item:any, idx) => {
                                return <Link key={idx} href={`/contratos/${item.id}`}>
...
export const getServerSideProps: GetServerSideProps = async () => {
    let urlCategorias = "https://webio.aparecida.go.gov.br/api/lict/contratos/categorias";
    const resCategoria = await fetch(urlCategorias);
    const dataCategoria = await resCategoria.json()

    let urlFornecedores = "https://webio.aparecida.go.gov.br/api/lict/fornecedores";
    const resFornecedor = await fetch(urlFornecedores);
    const dataFornecedor = await resFornecedor.json()

    let urlEntidades = "https://webio.aparecida.go.gov.br/api/lict/licitacoes/entidades";
    const resEntidade = await fetch(urlEntidades);
    const dataEntidade = await resEntidade.json()
  
    return {
       props: {
        categorias: dataCategoria,
        fornecedores: dataFornecedor,
        entidades: dataEntidade,
       } 
    }
  }
